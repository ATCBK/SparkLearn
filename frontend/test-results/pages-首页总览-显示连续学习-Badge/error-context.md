...
- Name: pages.spec.ts >> 首页总览 >> 显示连续学习 Badge
- Location: test\pages.spec.ts:160:7
...
Locator: getByText(/连续学习/)
Expected: visible
Error: strict mode violation: getByText(/连续学习/) resolved to 2 elements:
    1) <span class="mt-0.5 block text-micro text-muted">连续学习</span> aka getByText('连续学习').first()
    2) <span class="mt-1 block text-micro leading-5 text-muted">连续学习</span> aka getByText('连续学习').nth(1)

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText(/连续学习/)
...
```ts
  ...
  66  |     await expect(page.getByText('期末提分')).toBeVisible()
  67  |     await expect(page.getByText('竞赛准备')).toBeVisible()
  68  |     await expect(page.getByText('兴趣探索')).toBeVisible()
  69  |     await expect(page.getByText('求职准备')).toBeVisible()
  70  |   })
  71  | 
