// Expose init globally for dynamic loading, prevent double init
var __dhLiveInited = false;
window.__dhLiveInit = function() {
  if (__dhLiveInited) return;
  __dhLiveInited = true;
  return init();
};
...
async function init()
{
            ...
            try {
              ...
              console.error('[DH_live] init error:', e);
              if (spinner) spinner.textContent = '加载失败: ' + e.message;
            }
}
