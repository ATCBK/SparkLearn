...
import type {
  ...
  LearningPath, VideoInfo, VideoStyle, ContributionDay, TutorRole, TutorConversation, TutorFile, PathNodeAdvice, PathAdjustResult, WorkshopHubEvent, ProfileUpdatePayload, KnowledgeFile, KnowledgeStats, TaskCreatePayload,
...
export async function generateResource(
  ...
  videoOptions?: { durationSec?: number; targetLevel?: string; styleId?: string; onEvent?: (evt: { type: string; payload: Record<string, unknown> }) => void },
): Promise<Resource> {
  // 视频类型走专用生成管线
  if (type === 'video') {
    return generateVideo(prompt, videoOptions?.onEvent, {
      durationSec: videoOptions?.durationSec,
      targetLevel: videoOptions?.targetLevel,
      styleId: videoOptions?.styleId,
    })
  }
...
export async function polishVideoPrompt(
  prompt: string,
  durationSec: number = 90,
  targetLevel: string = 'beginner',
  styleId: string = 'apple-minimal',
): Promise<{
  polishId: string
  title: string
...
export async function generateVideo(
  ...
  options?: { durationSec?: number; targetLevel?: string; styleId?: string },
): Promise<Resource> {
  const durationSec = options?.durationSec ?? 90
  const targetLevel = options?.targetLevel ?? 'beginner'
  const styleId = options?.styleId ?? 'apple-minimal'

  // Step 1: Polish the prompt into a script
  const polish = await polishVideoPrompt(prompt, durationSec, targetLevel, styleId)
