...
export default function TeacherLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (username === 'teacher' && password === '123456') {
      ...
      setError('账号或密码错误，演示账号：teacher / 123456')
    }
    setLoading(false)
  }
  ...
  return (
    ...
    >
      <div className="w-full max-w-[400px] rounded-[16px] border border-[#e5e7eb] bg-white p-8 shadow-lg">
        {/* Logo */}
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold text-[#111827]">教师工作台</h1>
            <p className="mt-1 text-sm text-[#6b7280]">SparkLearn · 教师端</p>
          </div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {error && (
            <div className="rounded-[8px] bg-[#fef2f2] px-3 py-2.5 text-xs text-[#dc2626]">
              {error}
            </div>
          )}
