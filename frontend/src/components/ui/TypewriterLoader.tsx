...
interface TypewriterLoaderProps {
  text?: string
  /** Use "sm" for inline/button contexts, "md" (default) for page-level loading */
  size?: 'sm' | 'md'
}

export function TypewriterLoader({ text, size = 'md' }: TypewriterLoaderProps) {
  const scale = size === 'sm' ? 0.6 : 1

  return (
    <StyledWrapper className="flex flex-col items-center gap-4" $scale={scale}>
      <div className="typewriter" aria-label="loading">
        <div className="slide"><i /></div>
        <div className="paper" />
        <div className="keyboard" />
      </div>
      {text && <p className="text-ink-secondary text-small mt-2">{text}</p>}
    </StyledWrapper>
  )
}
