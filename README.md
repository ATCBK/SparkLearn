...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Nanobot-本机托管接入-10B981" />
  <img src="https://img.shields.io/badge/Tutor工作区-角色%2F文件%2F研讨-F43F5E" />
  <img src="https://img.shields.io/badge/MCP-插件商店与接口-EAB308" />
  <img src="https://img.shields.io/badge/RAG-知识检索增强-14B8A6" />
  <img src="https://img.shields.io/badge/向量化-Embedding检索-06B6D4" />
  <img src="https://img.shields.io/badge/记忆系统-多层记忆召回-6366F1" />
  <img src="https://img.shields.io/badge/防幻觉-可信回答控制器-F97316" />
  <img src="https://img.shields.io/badge/多模态对话-文本%2B图片生成-EC4899" />
</p>
...
学习宠物能力已具备接入本机 `nanobot` 内核的技术路径，支持在本地运行环境中完成宠物任务处理与结果回传。

- 接入方式：后端路由统一调度，本机服务托管。
- 运行形态：`backend` 与 `nanobot` 本机联调。
- 工程目标：在不改变学习宠物前端交互形态的前提下升级内核能力。

### 7) RAG 与文本向量化检索

知识问答链路已具备向量化检索能力，支持从知识文件分片中召回高相关证据片段。

- 向量化能力：Embedding（query/para）文本向量化。
- 检索能力：知识分片召回 + 上下文拼接。
- 目标：减少纯拼接回答，提高知识问答准确性与可解释性。

### 8) 记忆系统与多轮上下文

系统具备多层记忆存储与召回能力，用于支持连续学习对话与个体化辅导。

- 记忆层：working / episodic / semantic / perceptual。
- 能力：记忆写入、检索、压缩整合、遗忘清理。
- 目标：降低多轮对话信息丢失，提升连续性与个体化。

### 9) 可信回答与防幻觉机制

已接入可信回答控制思路，在知识问答场景中增强证据约束与可信度表达。

- 机制：检索证据优先、回答约束、可信度反馈。
- 场景：知识问答、学习指导、引用证据类回答。
- 目标：降低幻觉风险，提升回答可验证性与教学可信度。

### 10) 多模态对话链路

Tutor 场景已支持文本主链路与图片生成链路并行存在，并进行流程隔离。

- 形态：文本问答 + 图像生成。
- 策略：图片生成链路与可信问答链路解耦，避免互相干扰。
- 目标：在保证稳定性的前提下提供多模态学习体验。
...
| 数据 | SQLite + 文件存储 |
| AI 能力 | 讯飞星火、星辰Agent平台、多模态生成链路、Nanobot 本机接入、RAG、Embedding 向量检索、记忆系统、可信回答控制 |
...
| Tutor 学习工作区 | 角色管理、文件管理、知识页、研讨页 | 支持更结构化的人机协同学习流程 |
| MCP 插件商店入口 | 进入学习空间插件市场，扩展工具能力 | 支持后续工具生态扩展与能力装配 |
| 知识库与视频资源 | 主题检索、关联问答、视频化学习 | 支持不同学习风格与场景 |

学生端主路由示例：
- `/`、`/profile`、`/path`、`/generate`、`/practice`、`/report`、`/agent`、`/knowledge`、`/video`
- Tutor 工作区：`/tutor`、`/tutor/roles`、`/tutor/files`、`/tutor/knowledge`、`/tutor/workshop`
- 学习空间扩展：`/tutor/mcp-store`、`/tutor/mcp-store/docs`
...
- Tutor/学习 API：`/api/profile`、`/api/learning`、`/api/path`、`/api/quiz`、`/api/resources`、`/api/knowledge`。
- Agent API（`/api/agent`）：学习宠物、任务发起与任务状态查询。
- MCP API（`/api/mcp`）：插件商店相关接口与工具接入入口。
- Video/Voice API：`/api/video`、`/api/voice-admin`。
- Memory API（`/api/memory`）：记忆读写、检索、整理、遗忘。
- Knowledge/RAG API（`/api/knowledge`）：知识文件、分片检索、上下文召回。
...
- 学习宠物支持本机 `nanobot` 接入链路（用于本地内核升级与联调）。
- 学习空间已加入 MCP 插件商店页面入口（后续扩展插件生态）。
- Tutor 学习工作区已加入角色、文件、知识与研讨页面，支持更完整的学习协作流。
- 已加入 RAG + Embedding 文本向量化检索链路，用于知识问答增强。
- 已加入多层记忆系统（working/episodic/semantic/perceptual）用于多轮上下文持续。
- 已加入可信回答与防幻觉机制，并支持多模态对话链路（文本/图片）。
